#include "tractionForceAdjointPOutletFvPatchScalarField.H"
#include "volFields.H"
#include "addToRunTimeSelectionTable.H"
#include "surfaceFields.H"
#include "fvPatchFieldMapper.H"
#include "fvc.H"


Foam::tractionForceAdjointPOutletFvPatchScalarField::
tractionForceAdjointPOutletFvPatchScalarField
(
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF
)
:
    fixedValueFvPatchScalarField(p, iF)
{}


Foam::tractionForceAdjointPOutletFvPatchScalarField::
tractionForceAdjointPOutletFvPatchScalarField
(
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
    const dictionary& dict
)
:
    fixedValueFvPatchScalarField(p, iF)
{
    fvPatchScalarField::operator=(scalarField("value", dict, p.size()));
}


Foam::tractionForceAdjointPOutletFvPatchScalarField::
tractionForceAdjointPOutletFvPatchScalarField
(
    const tractionForceAdjointPOutletFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
    const fvPatchFieldMapper& mapper
)
:
    fixedValueFvPatchScalarField(ptf, p, iF, mapper)
{}


Foam::tractionForceAdjointPOutletFvPatchScalarField::
tractionForceAdjointPOutletFvPatchScalarField
(
    const tractionForceAdjointPOutletFvPatchScalarField& pivpvf,
    const DimensionedField<scalar, volMesh>& iF
)
:
    fixedValueFvPatchScalarField(pivpvf, iF)
{}


// Update the coefficients associated with the patch field
void Foam::tractionForceAdjointPOutletFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    }

    const fvPatchField<vector>& patchU =
        patch().lookupPatchField<volVectorField, scalar>("U");

    scalarField patchUNormal(mag(patch().nf() & patchU));

    const fvPatchField<vector>& patchAdjointU =
        patch().lookupPatchField<volVectorField, scalar>("adjointU");

    scalarField patchadjointUNormal(mag(patch().nf() & patchAdjointU));

    scalarField::operator=
    (
        (patchU & patchAdjointU) 
    );

    fixedValueFvPatchScalarField::updateCoeffs();
}


void 
Foam::tractionForceAdjointPOutletFvPatchScalarField::write(Ostream& os) const
{
    fvPatchScalarField::write(os);
    writeEntry("value", os);
}


namespace Foam
{
    makePatchTypeField
    (
        fvPatchScalarField,
        tractionForceAdjointPOutletFvPatchScalarField
    );
}

